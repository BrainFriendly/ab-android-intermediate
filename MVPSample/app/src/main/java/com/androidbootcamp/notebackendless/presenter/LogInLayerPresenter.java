...
public class LogInLayerPresenter {
    ...
    private void authentication(){
        view.showLoading();
        Injection.getProvider().logIn(view.getUsername(), view.getPassword(),
                new RemoteCallback() {
                    @Override
                    public void onSuccess(Object result) {
                        view.hideLoading();
                        if(result instanceof LogInBLResponse){
                            LogInBLResponse response= (LogInBLResponse)(result);
                            view.saveSession(response);
                            view.gotoMain();
                        }
                    }
